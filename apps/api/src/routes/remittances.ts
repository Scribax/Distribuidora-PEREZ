...
import { Prisma, RemitoEstado, Rol } from "@prisma/client";
...
remittancesRouter.get("/", async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    prisma.remito.findMany({ where, skip, take, include: { cliente: true, items: true }, orderBy: { numero: "desc" } }),
...
remittancesRouter.get("/:id", async (req, res) => {
  ...
  const remito = await prisma.remito.findUnique({
    where: { id },
    include: { cliente: true, items: true, usuario: { select: { nombre: true } } }
  });
...
remittancesRouter.put("/:id", requireRoles(Rol.ADMINISTRADOR, Rol.EMPLEADO), async (req, res) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    return tx.remito.update({
      where: { id: remito.id },
      data: { total: newItems.reduce((sum, item) => sum + item.subtotal, 0) },
      include: { items: true, cliente: true }
    });
  });
  res.json(updated);
...
remittancesRouter.get("/:id/pdf", async (req, res) => {
  ...
  doc.fontSize(16).text(`Remito Nro. ${remito.numero}`, right - 190, 48, { width: 190, align: "right" });
  ...
  doc.fillColor("#166534").fontSize(13).text(`Saldo pendiente: ${money(remito.saldoClienteAlEmitir)}`, left + 16, 208);

  doc.roundedRect(left, 248, pageWidth, 118, 6).strokeColor("#d9e2dd").stroke();
  doc.fontSize(13).fillColor("#111111").text("Cliente", left + 16, 264);
  doc.fontSize(10);
  doc.fillColor("#496054").text("Nombre", left + 16, 288);
  doc.fillColor("#111111").text(remito.cliente.nombre, left + 78, 288, { width: 190 });
  doc.fillColor("#496054").text("Empresa", left + 300, 288);
  doc.fillColor("#111111").text(remito.cliente.empresa ?? "-", left + 365, 288, { width: 160 });
  doc.fillColor("#496054").text("Dirección", left + 16, 310);
  doc.fillColor("#111111").text(remito.cliente.direccion ?? "-", left + 78, 310, { width: 190 });
  doc.fillColor("#496054").text("Teléfono", left + 300, 310);
  doc.fillColor("#111111").text(remito.cliente.telefono ?? "-", left + 365, 310, { width: 160 });
  doc.fillColor("#496054").text("Email", left + 16, 332);
  doc.fillColor("#111111").text(remito.cliente.email ?? "-", left + 78, 332, { width: 430 });

  doc.fontSize(13).fillColor("#111111").text("Productos", left, 390);
  const startX = 48;
  let y = 416;
